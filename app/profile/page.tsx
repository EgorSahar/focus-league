...
export default function ProfilePage() {
    ...
    // Реестр всех юзеров и результаты поиска
    ...
    // --- 2. ОБНОВЛЕНИЕ ЛОКАЛЬНОГО РЕЕСТРА ---
    useEffect(() => {
    ...
    const connectTelegram = async () => {
        if (!userUid) return
        try {
            // 1. Умная настройка вебхука на сервере
            ...
            if (error) {
                alert('Ошибка базы данных! Убедись, что отключил RLS в Supabase. ' + error.message)
                return
            }
            ...
            alert('Ошибка при настройке Telegram. Проверь консоль.')
        }
    }
    ...
    return (
        <div style={{ minHeight: '100vh', backgroundColor: 'var(--bg-main)', color: 'var(--text-main)', fontFamily: 'sans-serif', paddingBottom: '100px', display: 'flex', flexDirection: 'column', alignItems: 'center', transition: 'background-color 0.3s' }}>
            ...
            {/* --- МОДАЛЬНОЕ ОКНО СОЦИАЛКИ --- */}
            <AnimatePresence>
                {isFriendsModalOpen && (
                    <div style={{ position: 'fixed', inset: 0, zIndex: 100, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsFriendsModalOpen(false)} style={{ position: 'absolute', inset: 0, backgroundColor: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(5px)' }} />
                            ...
                            {viewingUser ? (
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
                                    ...
                                    <div style={{ display: 'flex', backgroundColor: 'var(--bg-surface)', borderRadius: '16px', padding: '4px', marginBottom: '24px', border: '1px solid var(--border-main)', flexShrink: 0 }}>
                                        <button onClick={() => setActiveTab('followers')} style={{ flex: 1, padding: '10px 0', borderRadius: '12px', fontSize: '15px', fontWeight: 'bold', border: 'none', cursor: 'pointer', backgroundColor: activeTab === 'followers' ? 'var(--bg-main)' : 'transparent', color: activeTab === 'followers' ? 'var(--text-main)' : 'var(--text-secondary)', boxShadow: activeTab === 'followers' ? '0 2px 8px rgba(0,0,0,0.1)' : 'none', transition: 'all 0.2s' }}>
                                            Подписчики ({myFollowersList.length})
                                        </button>
                                        <button onClick={() => setActiveTab('following')} style={{ flex: 1, padding: '10px 0', borderRadius: '12px', fontSize: '15px', fontWeight: 'bold', border: 'none', cursor: 'pointer', backgroundColor: activeTab === 'following' ? 'var(--bg-main)' : 'transparent', color: activeTab === 'following' ? 'var(--text-main)' : 'var(--text-secondary)', boxShadow: activeTab === 'following' ? '0 2px 8px rgba(0,0,0,0.1)' : 'none', transition: 'all 0.2s' }}>
                                            ...
                                            <div style={{ display: 'flex', flexDirection: 'column' }}>
                                                {myFollowersList.length === 0 ? (
                                                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '16px', color: 'var(--text-secondary)', marginTop: '40px' }}>
                                                        ...
                                                        <p style={{ textAlign: 'center', fontSize: '16px' }}>У тебя пока нет подписчиков.<br />Делись своим ID, чтобы собрать Сквад!</p>
                                                    </div>
                                                ) : (
