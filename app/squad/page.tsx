...
import { Home, Trophy, Users, User, Flame, MessageSquare } from 'lucide-react'
...
export default function SquadPage() {
    ...
    useEffect(() => {
        const initData = async () => {
            ...
            // Загружаем посты и связываем их с профилем автора!
    ...
    // --- СОЦИАЛЬНЫЙ БУСТ (ЛАЙК = +10 XP АВТОРУ) ---
    const giveFire = async (postId: string, authorId: string, currentLikes: string[]) => {
        if (!userUid) return

        // Защита: нельзя лайкнуть дважды свой или чужой пост
        const likesArray = currentLikes || []
        if (likesArray.includes(userUid)) return

        const newLikes = [...likesArray, userUid]

        // 1. Оптимистично обновляем UI (чтобы кнопка сразу стала оранжевой)
        setPosts(posts.map(p => p.id === postId ? { ...p, likes: newLikes } : p))

        // 2. Обновляем массив лайков в самом посте
        await supabase.from('posts').update({ likes: newLikes }).eq('id', postId)

        // 3. МАГИЯ БУСТА: Начисляем автору поста +10 XP!
